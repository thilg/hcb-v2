 ...
 */
public class AdwordsBalanceAlgorithmDataGenerator extends Configured implements
		Tool {
		...
		public void map(Object key, Text value, Context context)
			...
			try {
				if (keyVal[0].startsWith("keyword:")) {
					context.write(new Text(keyVal[0].replace("keyword:", "")),
							new Text(keyVal[1]));
					...
					for (String[] bid : bids) {
						...
						context.write(new Text(keyword),
								new Text(new StringBuffer().append(clientid)
										.append(",").append(budget).append(",")
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			try {
				...
				for (String bidData : bids) {
					...
					budget = vals[1];
					bid = vals[2];
					buf.append(clientid).append(",").append(budget).append(",")
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(getConf(),
				"AdwordsBalanceAlgorithmDataGenerator");
